...
function runExport(repoRoot, options = {}) {
  ...
  throw new Error(formatError(translator.t('ai.error.unsupported_export_format', { format })));
}
...
function runActiveSlice(repoRoot, options = {}) {
  ...
  if (command !== 'status' && command !== 'reconcile') {
    throw new Error(formatError(translator.t('ai.error.unsupported_active_slice_subcommand', { command })));
  }
  if (command === 'reconcile' && options.dryRun !== true) {
    throw new Error(formatError(translator.t('ai.active_slice.error.reconcile_requires_dry_run')));
  }
...
function runLifecycleRun(repoRoot, options = {}) {
  ...
  const namespace = options.namespace === 'lifecycle' ? 'lifecycle' : 'run';
  const commandPrefix = `ai ${namespace}`;
  const errorPrefix = namespace === 'lifecycle' ? 'ai.lifecycle.error' : 'ai.run.error';
  if (command !== 'create' && command !== 'close') {
    throw new Error(formatError(translator.t(`${errorPrefix}.unsupported_subcommand`, { command })));
  }
  if (command === 'create' && !options.input) {
    throw new Error(formatError(translator.t(`${errorPrefix}.create_requires_input`)));
  }
  if (command === 'close') {
    const current = resolveAiRun(repoRoot, options.runId || '');
    if (!current) {
      throw new Error(formatError(translator.t(`${errorPrefix}.close_requires_run`)));
    }
    const run = updateAiRunPhase(repoRoot, current.run_id, 'closed', {
      command: `${commandPrefix} close`,
    });
