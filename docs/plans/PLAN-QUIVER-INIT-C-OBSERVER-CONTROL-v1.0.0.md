...
requirements:
  - artifact_id: "REQ-QUIVER-INIT-C-OBSERVER-CONTROL"
    version: "1.0.1"
    path: "../requirements/initiatives/REQ-QUIVER-INIT-C-OBSERVER-CONTROL-v1.0.1.md"
parent_plan:
  artifact_id: "PLAN-QUIVER-MASTER"
  version: "6.0.7"
  path: "./PLAN-QUIVER-MASTER-v6.0.7.md"
decisions:
...
entrega verificable para `SPEC-V71` a `SPEC-V76`. Su estado es `proposed` y
requiere aprobación explícita del project owner antes de materializar specs o
iniciar implementación.

La aprobación futura de este documento:

- congelará el orden, los gates y las obligaciones de evidencia de C;
- habilitará materializar specs y slices solo después de cerrar sus dependencias
  técnicas y comerciales;
- habilitará continuar documentalmente con el plan de la iniciativa D;
- no aprobará G1 o G2 ni iniciará código, integraciones, enforcement, release o
  deploy.
...
- Las cadenas documentales A y B están aprobadas.
...
- Done sin evidencia, PR abierto y trabajo sin issue/requirement conservan su
  condición real según policy;
...
- V71: duplicar, desordenar y omitir eventos GitHub; reconciliar el estado y
  comprobar que ninguna operación de escritura es posible.
- V72: correlacionar Linear↔GitHub con granularidad distinta, actualización
  externa y divergencia, sin loops ni writes.
- V73: producir una causa repetida, reconciliar un único finding y recorrer
  resolución/aceptación/descarte con actor y razón.
- V74: trazar un incidente hasta su SHA y mantener `unknown` cuando source maps
  o release no lo demuestran, sin rollback/promoción.
...
tres elementos de la cadena y aceptar solamente el binding de C:

1. `REQ-QUIVER-INIT-C-OBSERVER-CONTROL@1.0.1`;
2. el binding de C de `PLAN-QUIVER-MASTER@6.0.7`, sin aprobar el roadmap completo
   ni las iniciativas D–G;
3. `PLAN-QUIVER-INIT-C-OBSERVER-CONTROL@1.0.0`.

Frase sugerida:

> Apruebo la cadena documental C: REQ-C 1.0.1, binding maestro 6.0.7 y PLAN-C
> 1.0.0.

La aprobación deberá persistirse como sucesores versionados en el orden
requirement → master/catalog binding → plan. No será transitive, no aprobará G1
o G2, no iniciará implementación y no habilitará crear el plan D hasta quedar
