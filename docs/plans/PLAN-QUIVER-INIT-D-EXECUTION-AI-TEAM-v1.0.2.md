...
owner: "Fabri Juncal"
date: "2026-09-03"
supersedes: "./PLAN-QUIVER-INIT-D-EXECUTION-AI-TEAM-v1.0.1.md"
requirements:
  - artifact_id: "REQ-QUIVER-INIT-D-EXECUTION-AI-TEAM"
    version: "1.0.2"
    path: "../requirements/initiatives/REQ-QUIVER-INIT-D-EXECUTION-AI-TEAM-v1.0.2.md"
parent_plan:
  artifact_id: "PLAN-QUIVER-MASTER"
  version: "6.0.12"
  path: "./PLAN-QUIVER-MASTER-v6.0.12.md"
decisions:
  ...
  - decision_id: "DEC-20260903-050"
    date: "2026-09-03"
    actor: "technical-agent"
    change: "Resolver PLAN-D-REV-01 a PLAN-D-REV-03"
    reason: "Cerrar el binding y replay de autorizaciones, los efectos tardíos de cancelación y la variable controlada de evals"
    impact: "Refuerza gates y pruebas de V77, V78 y V80; no cambia los 46 RQs ni aprueba G3"
  - decision_id: "DEC-20260903-055"
    date: "2026-09-03"
    actor: "project-owner"
    change: "Aprobar el plan de la iniciativa D"
    reason: "Aprobación explícita de REQ-D 1.0.1, binding maestro 6.0.11 y PLAN-D 1.0.1"
    impact: "Crea PLAN-D 1.0.2 aprobado y habilita planificar E; no inicia implementación ni aprueba G3/G4"
---
...
entrega verificable para `SPEC-V77` a `SPEC-V81`. Su estado es `approved` por
decisión explícita del project owner.

La aprobación registrada de este documento:

- congela el orden, los gates y las obligaciones de evidencia de D;
- habilita materializar specs y slices solo después de cerrar G3 y las
  dependencias técnicas aplicables;
- habilita continuar documentalmente con el plan de la iniciativa E;
- no aprueba G3/G4 ni inicia ejecución remota, agentes, integraciones, código,
  release o deploy.
...
10. Congelar el registro de autorización para approvals y grants: organización,
    proyecto, run, subject/recipient, authorizer, acción/recurso exacto o digest,
    versión de envelope/policy, expiración y contador de uso acotado.
11. Definir límites de runs, cancelación/kill switch y stop conditions. El
    lifecycle será monotónico y cada run tendrá generación/epoch; cancelar lo
    lleva a estado terminal e invalida la generación antes de aceptar resultados
    o habilitar nueva ejecución delegada.
...
- el runtime puede reportar resultado, pero solo la autoridad contractual de
  Quiver decide `DONE`;
- cancelar realiza una transición monotónica a estado terminal, invalida la
  generación/epoch activa, detiene nuevas acciones y revoca credenciales cuando
  sea posible;
- cada evento, artifact, resultado, write o cargo recibido verifica estado y
  generación vigentes antes de adquirir autoridad contractual;
- una finalización tardía posterior a cancelación se rechaza o queda en
  cuarentena diagnóstica: no valida evidencia, no cambia `DONE`, no escribe ni
  autoriza costo nuevo; el audit trail y el costo ya incurrido sí se conservan
  para reconciliación;
...
- cada approval o grant puntual usa un registro inmutable ligado a organización,
  proyecto, run, subject/recipient, authorizer, acción/recurso exacto o digest,
  versión de envelope/policy, razón, expiración y contador de uso acotado;
- el registro se valida antes del side effect y nunca reescribe ni amplía el
  envelope original;
- una autorización cruzada, stale, repetida, fuera de scope, expirada o con usos
  agotados se rechaza sin mutación;
...
- cada comparación controlada varía exactamente una dimensión candidata y fija
  las demás entradas aplicables y compatibles; cuando se compara runtime, el
  runtime es la variable, y toda diferencia inevitable del proveedor se registra
  y limita la interpretación;
...
| Approval/grant cruzado, repetido o fuera de scope habilita un side effect | Media | Crítico | Casos cross-run/subject, replay, expiry, uso agotado y acción distinta | Binding inmutable y validación previa sin mutación |
| Resultado tardío tras cancelación altera estado o produce efectos | Media | Crítico | Comando demorado que completa después del estado terminal | Generación monotónica, rechazo o cuarentena diagnóstica |
...
- V77: ejecutar la misma fixture con dos adapters; verificar capability ausente,
  normalización/redacción y aislamiento de path/workspace/tenant. Demorar un
  comando hasta después de cancelar y comprobar que la generación terminal
  impide aceptar su resultado como artifact, write, evidencia, `DONE` o nueva
  autorización de costo.
- V78: negar filesystem/command/network/tool/secret/producción fuera de envelope;
  probar approval/grant cross-run y cross-subject, stale, replayed, para otra
  acción/recurso, expirado y con usos agotados, siempre antes del side effect y
  sin mutación; probar además checkpoint con SHA/config distinto, dos claims
...
- V80: alterar lock, ejecutar hard failure con model grader favorable y bloquear
  una regresión sobre el umbral aprobado. Comparar dos runtimes variando solo el
  runtime, fijar las demás entradas compatibles y registrar las diferencias de
  proveedor inevitables.
...
- runs activos/cancelados, generaciones invalidadas, resultados tardíos en
  cuarentena, capabilities ausentes y fallos de adapters;
- violaciones de workspace/envelope, approvals/grants cruzados, replayed, stale,
  agotados y revocaciones;
...
- side effect fuera del Permission Envelope o autorizado por un approval/grant
  cruzado, stale, repetido, fuera de scope, expirado o agotado;
- resultado tardío aceptado como evidencia/`DONE`, write o nueva autorización de
  costo después de cancelar;
...
## 12. Cierre de hallazgos de revisión

| Hallazgo | Ajuste incorporado | Criterio verificable de cierre |
|---|---|---|
| `PLAN-D-REV-01` | Binding inmutable y validación previa de approvals/grants | Casos cruzados, stale, replayed, fuera de scope, expirados o agotados se rechazan sin side effect ni mutación |
| `PLAN-D-REV-02` | Estado terminal y generación/epoch monotónicos al cancelar | Una finalización demorada queda rechazada o diagnóstica y no altera artifacts, writes, evidencia, `DONE` ni autoriza costo nuevo |
| `PLAN-D-REV-03` | Matriz de comparación de una variable por vez | La dimensión candidata varía, las entradas compatibles restantes quedan fijas y las diferencias inevitables se registran |

Estos ajustes aclaran controles y validaciones del alcance existente. No agregan
RQs, no alteran la precedencia y no constituyen aprobación de G3.

## 13. Cadena de aprobación registrada

El project owner aprobó explícitamente la cadena propuesta:

1. `REQ-QUIVER-INIT-D-EXECUTION-AI-TEAM@1.0.1`, persistido como sucesor
   aprobado `1.0.2`;
2. el binding de D de `PLAN-QUIVER-MASTER@6.0.11`, actualizado en `6.0.12`
   sin aprobar el roadmap completo, las iniciativas E–G, G3 o G4;
3. `PLAN-QUIVER-INIT-D-EXECUTION-AI-TEAM@1.0.1`, persistido como este sucesor
   aprobado `1.0.2`.

La aprobación habilita crear el plan E. No inicia código, specs, ejecución
remota, integraciones, release o deploy, no aprueba G3/G4 y no afirma que las
dependencias de ejecución de E estén completadas.
