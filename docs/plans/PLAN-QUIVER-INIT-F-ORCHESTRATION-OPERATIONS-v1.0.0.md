...
parent_plan:
  ...
  version: "6.0.16"
  path: "./PLAN-QUIVER-MASTER-v6.0.16.md"
decisions:
...
4. `DO_NOT_BUILD` cierra V87 sin tratar esa salida como fallo; la reevaluación
   exige el trigger definido en el ADR.
...
```text
P0
├─ P1 V86 ─ G5 ─┬─ DO_NOT_BUILD ─ cerrar V87
...
- G5 queda como decisión separada del project owner y puede concluir
  `DO_NOT_BUILD` sin habilitar V87.
...
1. detener evaluación, ingestion, scheduler, workers y side effects del alcance
   afectado;
2. invalidar generation/epoch, approvals, leases y handles; revocar credenciales
   cuando sea posible;
3. aislar tenant, environment, integration e incident source y preservar audit
   trail;
4. reconciliar estado durable contra efectos externos antes de retry o resume;
5. deshabilitar adapter, workflow o automatización responsable y volver al
   mecanismo manual o backend anterior aprobado;
6. bloquear auto-merge/rollback y devolver el incidente al owner humano;
7. versionar ADR, policy, spec o plan antes de reanudar; no borrar evidencia útil
   para auditoría o costo;
8. abrir incidente y aplicar el Feature Delivery Loop cuando exista defecto.
...
2. el binding de F de `PLAN-QUIVER-MASTER@6.0.16`, sin aprobar el roadmap
   completo, la iniciativa G o G5;
3. `PLAN-QUIVER-INIT-F-ORCHESTRATION-OPERATIONS@1.0.0`.

Frase sugerida:

> Apruebo la cadena documental F: REQ-F 1.0.1, binding maestro 6.0.16 y PLAN-F
> 1.0.0.
