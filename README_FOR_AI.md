...
The v31 spec is in progress under `specs/quiver-v31-ai-model-catalog-agent-selection/`; slice-00 through slice-03 are completed. Current implemented scope includes the local AI model catalog, alias normalization, guided provider/model selection for `ai agent set <role>`, `ai agent doctor`, and `ai agent repair --dry-run`. Remaining planned scope includes `ai models list`, shared live-command model preflight, and clearer provider errors for invalid model ids.
