...
The v31 spec is planned under `specs/quiver-v31-ai-model-catalog-agent-selection/`; it targets a local AI model catalog, guided provider/model selection for agent profiles, alias normalization, `ai models list`, `ai agent doctor`, `ai agent repair --dry-run`, shared live-command model preflight, and clearer provider errors for invalid model ids.
