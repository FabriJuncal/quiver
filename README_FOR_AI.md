...
The v27 spec is implemented and validated under `specs/quiver-v27-reliability-ai-workflow-hardening/`; it captures Pixel Quiver dogfooding findings (`QP-001` to `QP-019`, `QIS-001` to `QIS-022`). Slices `slice-00` through `slice-09` are completed with full tests, smoke suites, fixture coverage, package/tarball smoke, and release-readiness evidence. Do not claim v27 is published until npm publication actually happens.
