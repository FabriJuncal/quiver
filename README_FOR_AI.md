...
The v27 spec is active under `specs/quiver-v27-reliability-ai-workflow-hardening/`; it captures Pixel Quiver dogfooding findings (`QP-001` to `QP-019`, `QIS-001` to `QIS-022`). Slices `slice-00` through `slice-07` are implemented and validated; continue from the current recommended slice in `STATUS.md`. Do not claim v27 is fully implemented or released until every slice is executed and validated.
...
`npx create-quiver analyze --dry-run` must remain read-only and only preview the scan/project-map/context writes. `npx create-quiver flow` reports the context source/freshness so agents can see whether the project map is current, stale, partial, legacy, invalid, or missing.
