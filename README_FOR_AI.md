...
The v31 spec is in progress under `specs/quiver-v31-ai-model-catalog-agent-selection/`; slice-00 through slice-05 are completed. Current implemented scope includes the local AI model catalog, alias normalization, guided provider/model selection for `ai agent set <role>`, `ai agent doctor`, `ai agent repair --dry-run`, shared live-command model preflight, clearer provider errors for invalid model ids, and `ai models list`. Remaining planned scope includes docs/templates alignment and final release readiness.
