...
If the project already exists from an older Quiver version and was previously initialized by Quiver, preview with `npx create-quiver migrate --dry-run`, apply a confirmed `migrate`, and verify independently with `npx create-quiver doctor --json` before `analyze` from the project root. Reapplying a current migration must return `already-current` without writes.
...
The v58 governance contract lives under `specs/quiver-v58-risk-aware-review-governance/`. It defines risk-aware profiles, canonical findings, bounded review budgets, digest-bound decisions, conditioned propagation, compatibility migration, and fail-closed rollback. Its presence in this repository does not assert npm publication.
...
Do not confuse `approval_recommendation: approve-with-risk` with the final `approved-with-conditions` decision. Governed findings stay run-scoped and visible; transfer or disposition requires canonical targets, explicit evidence, authorization, and exact criterion binding before a conditioned decision can be published.
The effective governance profile is `fast-delivery` or `high-assurance`; sensitive requirement categories force `high-assurance`, and an active run cannot be silently downgraded. When the review budget is exhausted, preserve `REVIEW_BUDGET_EXHAUSTED` plus `HUMAN_DECISION_REQUIRED` and use only the governed actions surfaced by Quiver.
...
Use `npx create-quiver ai approval show|verify|export` for canonical decision inspection and `npx create-quiver findings transfer|disposition` for governed finding mutations. Approval, status, flow, resume, export, spec generation, and slice/PR gates must consume the same canonical projection; JSON stdout stays free of human prose and machine keys, enums, statuses, codes, and exits are never localized.
AI lifecycle runs are persisted under `.quiver/runs/<run-id>/`; use `npx create-quiver ai run create --input docs/requirements/<requirement-id>-v<version>.md` to start one explicitly, `npx create-quiver ai status` to inspect it, `npx create-quiver ai resume` to continue from the last valid phase without relying on chat memory, and `npx create-quiver ai export --format json|markdown` when another agent, PR, or dashboard needs the current specs/slices/runs state.
Compatibility metadata lives at `governance.compatibility` in `.quiver/config.json`. `writer_mode: read-only` is the non-destructive rollback mode: readers and gates remain fail-closed while writers return `GOVERNANCE_READ_ONLY`. Never lower `minimum_writer_version` or rewrite governed records into legacy shapes. For `LEGACY_EVIDENCE_UNVERIFIED`, `GOVERNANCE_READ_ONLY`, `UNSAFE_WRITER_DOWNGRADE`, or `MIGRATION_VERIFICATION_FAILED`, follow `docs/TROUBLESHOOTING.md`.
...
12. Use `npx create-quiver ai plan --phase acceptance --input docs/requirements/<requirement-id>-v<version>.md --dry-run`; use `--print-prompt` when you need the exact prompt without provider auth and `--review --interactive` when the human should edit or confirm before saving
...
16. After human approval, save the reviewed plan version with `npx create-quiver ai approve --phase technical-plan` in TTY or `--version <n>` in CI/no-TTY; if review recommends `revise`, run `ai revise --phase technical-plan --input feedback.md --dry-run` first. Then persist the durable plan under `docs/plans/`, pin the exact requirement versions, link it from the requirement, and record the decision reason and impact before creating the spec
