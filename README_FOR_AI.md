...
The v27 spec is planned under `specs/quiver-v27-reliability-ai-workflow-hardening/`; it captures Pixel Quiver dogfooding findings (`QP-001` to `QP-019`, `QIS-001` to `QIS-022`) and must start with `slice-00-docs-audit-coverage-and-contracts` before implementation. Do not claim v27 is implemented until its slices are executed and validated.
