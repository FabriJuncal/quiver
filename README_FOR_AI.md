...
The v49 parser path uses an internal command registry plus compatibility adapter. `--help` annotates option ownership with `scope`; do not describe scoped flags as global. Quiver does not generate shell completion scripts in v49.
...
Use `npx create-quiver evidence run -- <command>` to capture validation evidence with command, exit code, duration, truncated output, and best-effort redaction. The safe default output is `.quiver/evidence/`; use `--output <file>` when a slice needs a specific evidence artifact. Use `npx create-quiver evidence list` and `npx create-quiver evidence show <path.md>` to browse captured evidence safely.
Use `npx create-quiver demo spec-viewer --dry-run` to inspect the optional Quiver Spec Viewer demo scaffold, then add `--dir <target>` for a real run. `demo create spec-viewer` remains compatible. The demo is not part of default init and must remain small, static, dependency-light, and non-destructive.
Use `npx create-quiver status` for a compact read-only state check and `npx create-quiver changelog` for local contract/version changes.
...
AI lifecycle runs are persisted under `.quiver/runs/<run-id>/`; use `npx create-quiver ai lifecycle create --input <requirements.md>` to start one explicitly, `npx create-quiver ai status` to inspect it, `npx create-quiver ai resume` to continue from the last valid phase without relying on chat memory, and `npx create-quiver ai export --format json|markdown` when another agent, PR, or dashboard needs the current specs/slices/runs state. `npx create-quiver ai run create --input <requirements.md>` remains a compatibility form.
Use advanced diagnostic command `npx create-quiver ai active-slice reconcile --dry-run` when `docs/ai/ACTIVE_SLICE.md`, `ACTIVE_SLICES.md`, worktrees, or `ai inspect` appear out of sync. The command is dry-run-first and reports preserve, close, replace, or blocked decisions without writing files.
...
If a new bounded transfer is needed, scaffold `specs/<project-slug>/HANDOFF.md` with `npx create-quiver handoff create <spec-slug>` and validate it with `npx create-quiver handoff check specs/<project-slug>/HANDOFF.md`.
Use `npx create-quiver handoff check specs/<project-slug>/HANDOFF.md` to validate a transferred handoff before execution.
Use `npx create-quiver slice check --local <slice.json>` for structural validation in a new repo without remote/base branches; run normal `slice check` before PR readiness so base/remote checks still happen.
...
- **Handoff scaffold:** if no handoff exists yet and the work needs one, use `npx create-quiver handoff create <spec-slug>` first.
...
- Generated project npm scripts should prefer `quiver:*` names such as `quiver:version`, `quiver:analyze`, `quiver:flow`, `quiver:dashboard`, `quiver:plan`, `quiver:graph`, `quiver:next`, `quiver:doctor`, `quiver:evidence`, `quiver:ai:agent`, `quiver:ai:inspect`, `quiver:ai:export`, `quiver:ai:specs`, `quiver:ai:slices`, `quiver:ai:trace`, `quiver:ai:prepare-context`, `quiver:ai:plan`, `quiver:ai:review-plan`, `quiver:ai:approve`, `quiver:ai:prompt-slice`, `quiver:ai:execute-slice`, `quiver:ai:execute-plan`, `quiver:spec:create`, `quiver:spec:start`, `quiver:spec:status`, `quiver:spec:validate`, `quiver:spec:close`, `quiver:slice:start`, `quiver:slice:check`, `quiver:slice:check-pr`, `quiver:slice:scope`, `quiver:slice:cleanup`, `quiver:slice:refresh`, `quiver:handoff:check`, and `quiver:handoff:create`. Legacy script aliases such as `quiver:start-slice`, `quiver:check-slice`, `quiver:check-pr`, `quiver:check-handoff`, and `quiver:new-handoff` remain available for compatibility.
- Optional demos are created with `npx create-quiver demo spec-viewer`; `demo create spec-viewer` remains compatible. Do not add demo output to the package or default init flow.
...
Bootstrap note: `slice start` should resolve paths canonically, prefer a local `develop` or `main` base branch before reaching for `origin`, and reject `draft` slices unless `--allow-draft` is passed intentionally. `start-slice` remains a compatibility alias.
