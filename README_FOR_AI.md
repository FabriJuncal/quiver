...
The v31 spec is in progress under `specs/quiver-v31-ai-model-catalog-agent-selection/`; slice-00 through slice-04 are completed. Current implemented scope includes the local AI model catalog, alias normalization, guided provider/model selection for `ai agent set <role>`, `ai agent doctor`, `ai agent repair --dry-run`, shared live-command model preflight, and clearer provider errors for invalid model ids. Remaining planned scope includes `ai models list` and docs/templates alignment.
